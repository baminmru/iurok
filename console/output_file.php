<?php
include_once(dirname(__FILE__)  . '/application/libraries/' .'iu_config.php');
		...
		function logWrite($msg)
		{
			...
			if($dconfig['log'])
			{
				...
				file_put_contents($dconfig['logpath'].'/_debug.txt', $date.'  '.$msg."\r\n", FILE_APPEND);
			}
			//echo($msg);
		}
		...
		$origname = isset($_REQUEST['origname']) ? $_REQUEST['origname'] : $id;
		
		
		global $dconfig;
	  // connect to DB
		$db = new mysqli($dconfig['db']['server'], $dconfig['db']['username'], $dconfig['db']['password'],
							   $dconfig['db']['database']);
		if ($db->connect_error) {
		...
		if ($origname==$id){
			$orig = '';
			$stmt = $db->prepare("SELECT origname from origname where filename='".$origname."'");
			$stmt->execute();
			$stmt->bind_result($orig);
			if ($stmt->fetch()) {
				$origname=$orig;
			}
			$stmt->close();
		...
		$path = $dconfig['storage'] . DIRECTORY_SEPARATOR .  $prefix.'_files'. DIRECTORY_SEPARATOR .$id;
