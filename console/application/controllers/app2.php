...
class App2 extends CI_Controller
{
    ...
    {      
	  ...
	  function MartSetFavorite()
    {      
        $instanceid = $this->input->get_post('instanceid', TRUE);
        $value = $this->input->get_post('value', TRUE);
        $this->martservice->get(array('Action' => 'SetFavorite',  'instanceid' => $instanceid, 'value'=>$value));
    }

    function MartResetFavorite()
    {      
        $instanceid = $this->input->get_post('instanceid', TRUE);
        $type = $this->input->get_post('type', TRUE);
        $this->martservice->get(array('Action' => 'ResetFavorite', 'type'=>$type));
    }
