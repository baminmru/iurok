...
function DefineInterface_iu_rcfg_mod_edit(id,mystore){
...
local: true 
 ...
 var p1;
    ...
    function onAddClick(){
   	if(CheckOperation('iu_rcfg.edit')!=0){
                var edit = Ext.create('EditWindow_iu_rcfg_modedit');
...
function DefineForms_iu_rcfg_mod_edit(){
...
Ext.define('Form_iu_rcfg_modedit', {
extend:  'Ext.form.Panel',
alias: 'widget.f_iu_rcfg_modedit',
...
listeners:{  select: function ( combo, records, eOpts ) {
		combo.up('form' ).activeRecord.set('moduleaccessible', records[0].get('value'));  
		var pvid = combo.up('form' ).down('#iu_rcfg_mod-1');
		if(combo.up('form' ).activeRecord.get('visiblecontrol')==-1){
			if (records[0].get('value')	==-1)
				pvid.setDisabled(false);
			else
				pvid.setDisabled(true);
		}else{
			pvid.setDisabled(true);
		}
...
allowBlank:false
    ...
    setActiveRecord: function(record,instid){
        ...
        if (record) {
			var pvid = this.down('#iu_rcfg_mod-1');
			if(record.get('visiblecontrol')==0 || record.get('moduleaccessible')==0 ){
				pvid.setDisabled(true);
			}
...
Ext.define('EditWindow_iu_rcfg_modedit', {
