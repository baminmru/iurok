...
function DefineInterface_iu_tm_act_(id,mystore){
...
var grid_iu_tm_actfile;
    function ChildOnDeleteConfirm(selection){
     ...
     function ChildOnDeleteClick(){
	 	if(grid_iu_tm_actfile.parentid=='{00000000-0000-0000-0000-000000000000}'){
			return;
		}
    ...
    function ChildOnAddClick(){
   	...
   	if(CheckOperation('iu_tm.edit')!=0){
                var edit = Ext.create('EditWindow_iu_tm_actfile');
...
var p1;
    ...
    function onAddClick(){
   	if(CheckOperation('iu_tm.edit')!=0){
                var edit = Ext.create('EditWindow_iu_tm_act');
...
tpl:'<a href=\'/output_file.php?ID={aktfile}&ext={aktfile_ext}\' target=\'_blank\'>Файл</a>'}
            ,
{text: "Дата оплаты", width:80, dataIndex: 'paymentdate', sortable: true, renderer:myDateRenderer}
            ,
...
function DefineForms_iu_tm_act_(){


Ext.define('Form_iu_tm_act', {
extend:  'Ext.form.Panel',
alias: 'widget.f_iu_tm_act',
...
Ext.define('EditWindow_iu_tm_act', {
...
Ext.define('Form_iu_tm_actfile', {
extend:  'Ext.form.Panel',
alias: 'widget.f_iu_tm_actfile',
initComponent: function(){
    this.addEvents('create');
    Ext.apply(this,{
        activeRecord: null,
...
allowBlank:true
        ...
        if (form.isValid()) {
        	...
        	StatusDB('Сохранение данных');
              form.submit({
               ...
               , success: function(){
						StatusReady('Изменения сохранены');
						active.store.load({params:{parentid:form.owner.parentid }});
						form.owner.ownerCt.close();
					 }
				, failure: function(response){
					StatusReady('Ошибка сохранения данных');
    ...
    onReset: function(){
        if(this.activeRecord.get('iu_tm_actfileid')==''){
                this.activeRecord.store.load({params:{parentid:this.parentid }});
        }
        this.setActiveRecord(null,null);
        this.ownerCt.close();
    }
...
Ext.define('EditWindow_iu_tm_actfile', {
    extend:  'Ext.window.Window',
    maxHeight: 318,
    maxWidth: 900,
    autoScroll:true,
