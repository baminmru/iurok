...
var groupingFeature_autoiu_rt_def = Ext.create('Ext.grid.feature.Grouping',{
groupByText:  'Группировать по этому полю',
showGroupsText:  'Показать группировку'
});
var interval_autoiu_rt_def;
Ext.define('grid_autoiu_rt_def', {
    extend:  'Ext.grid.Panel',
    alias: 'widget.g_v_autoiu_rt_def',
    requires: [
        'Ext.grid.*',
        'Ext.form.field.Text',
    ...
    initComponent: function(){
        Ext.apply(this, {
        frame: false,
        store: store_v_autoiu_rt_def,
        features: [groupingFeature_autoiu_rt_def],
        ...
        columns: [
            {text: "Название", width:800, dataIndex: 'iu_rt_def_name', sortable: true}
        ]
        ,
...
listeners: {render: function(e) {Ext.QuickTips.register({  target: e.getEl(), text: 'Название'});}}
    ...
    listeners: {
        ...
        ,
        	added:function(){
        			//interval_autoiu_rt_def= setInterval(function() {  
        			//	store_v_autoiu_rt_def.load();
        			//}, 60000);
        		}
        	,
        	destroy:function(){
        		//clearInterval(interval_autoiu_rt_def);
        }
    },
    onDeleteConfirm:function(selection){
      if (selection) {
        Ext.Ajax.request({
            url:    rootURL+'index.php/c_v_autoiu_rt_def/deleteRow',
            method:  'POST',
    		params: { 
    ...
    onAddClick: function(){
   	    if(CheckOperation('iud_rt.edit')!=0){
            Ext.Ajax.request({
                url: rootURL+'index.php/c_v_autoiu_rt_def/newRow',
                method:  'POST',
                params: { 
                },
                ...
                var edit = Ext.create('ObjectWindow_iud_rt');
                var p=iud_rt_Panel_( res.data, false,null ) ;
                edit.add(p);
                edit.show();
                }
            });
    ...
    onEditClick: function(){
        ...
        if (selection) {
   	    if(CheckOperation('iud_rt.edit')!=0){
                var edit = Ext.create('ObjectWindow_iud_rt');
            var p=iud_rt_Panel_( selection.get('instanceid'), false, selection ) ;
            edit.add(p);
            edit.show();
        }else{
        		Ext.MessageBox.show({
...
function iud_rt_Jrnl(){ 
  ...
  var iud_rt= Ext.create('Ext.form.Panel', {
      ...
      fieldDefaults: {
        ...
        items: [{
            itemId:'gr_autoiu_rt_def',
            xtype:'g_v_autoiu_rt_def',
            layout: 'fit',
            flex: 1,
            store: store_v_autoiu_rt_def
    }] // tabpanel
