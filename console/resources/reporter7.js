...
'Ext.form.*'
 ...
 var enum_ReportCKK = Ext.create('Ext.data.ArrayStore', {
  fields: [ {name: 'name'}, {name: 'value', type: 'string'} ], data: [ 
	//['Задачи. По процессам','CKKTaskXPrcFinished.mrt'],
	//['Задачи. По предметам','CKKTaskXSubjFinished.mrt'],
	['Задачи. По предметам и классам','CKKFinStatusXSubj.mrt'],
	['Статус. По процессам','CKKStateXPrcFinished.mrt']
 ]});
  ...
  var rpanel= Ext.create('Ext.form.Panel', {
      ...
      fieldDefaults: {
        ...
        items: [
		{
		 ...
		 title: 'Отчеты СКК',
		 reportfile:'',
		 html:'<iframe id="repCKK_frame_id" width="100%"  height="100%" src=""></iframe>',
		 rbar:
                [
                {
                    xtype:  'form',
                    ...
                    [
                        {
                            ...
                            handler: function() {
								...
								if(user_input['reportname']){
									...
									//alert(src);
									Ext.getElementById('repCKK_frame_id').src =encodeURI(src);
								}
