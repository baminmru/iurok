...
'Ext.form.*'
 ...
 var enum_ReportTask = Ext.create('Ext.data.ArrayStore', {
  fields: [ {name: 'name'}, {name: 'value', type: 'string'} ], data: [ 
	...
	,['Задачи. По исполнителям - Полученные','TaskXUser.mrt']
	,['Задачи. По исполнителям - Завершенные','TaskXUserFinished.mrt']
	//,['Задачи. СКК. По процессам - Завершенные','CKKTaskXUserFinished.mrt']
	,['Задачи. Подэтапы по исполнителям - Полученные','TaskStageUser.mrt']
	,['Задачи. Подэтапы по исполнителям - Завершенные','TaskStageUserFinished.mrt']
	,['Задачи. По месяцам - Полученные','TaskRoleMonth.mrt']
	,['Задачи. По месяцам - Завершенные','TaskRoleMonthFinished.mrt']
  ...
  var rpanel= Ext.create('Ext.form.Panel', {
      ...
      fieldDefaults: {
        ...
        items: [
		{
		 ...
		 reportfile:'',
		 html:'<iframe id="reptask_frame_id" width="100%"  height="100%" src=""></iframe>',
		 rbar:
                [
                {
                    xtype:  'form',
                    ...
                    [
                        {
                            ...
                            handler: function() {
								...
								if(user_input['reportname']){
									...
									//alert(src);
									Ext.getElementById('reptask_frame_id').src =encodeURI(src);
								}
