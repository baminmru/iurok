	  ...
	  try{
		  ...
		  if (Ext.isFunction(col.renderer)) {
			...
			var value = record.get(name),
				type  = this.typeMappings[col.type || record.fields.getByKey(name).type];
		  }
		  
		  cells.push(this.buildCell(value, type, style).render());
	  }catch(err){
		type = "String";
	    cells.push(this.buildCell("", type, style).render());
	  }
    }, this);
   ...
   buildTreeRow: function(record, index) {
    ...
    Ext.each(this.columns, function(col) {
	  try{
		...
		type = "String";
	    cells.push(this.buildCell(record[name], type, style).render());
	  }
    }, this);
