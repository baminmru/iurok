...
function taskCommentRenderer(value, metaData, record, row, col, store, gridView) 
	{
	...
	function taskCardRenderer(value, metaData, record, row, col, store, gridView) 
	{
	  if(metaData==null) return rootURL+'?uid='+ record.get('urokid');
	
	  return ('<a href="'+rootURL+'?uid='+ record.get('urokid')+'" target="_blank"><img src="'+rootURL+'/resources/icons/film_edit.png"></a>');
...
Ext.define('grid_autoiu_task', {
    ...
    initComponent: function(){
        ...
        columns: [
			...
			,
			*/
			{text: "Страница<br/>урока", width:80, renderer:taskCommentRenderer, sortable: false}
            ,	 
			{text: "Карточка<br/>урока", width:80, renderer:taskCardRenderer, sortable: false}
			,
