			...
			if(record.get('doctype')=='{BDFC8DA9-F226-11E3-8FBF-00155D0ED711}'  && record.get('activeversion')==-1 ){
				...
				if (record.get('fileurl')!=''){
					...
					if (record.get('fileref')!=''){
						...
						timing_text=record.get('filetext');
				...
				[
					{
						...
						html:'<video id="video_'+vid+'" class="video-js vjs-default-skin" controls preload="auto" width="640" height="360" data-setup="{}" >'+ 
					...
					{   xtype:'panel',
						...
						},
						{
							...
							handler:function(button,e)
							{
								  ...
								  var inf='';	
								  inf=button.up('panel').down('#timing'+vid).getValue();
								  
								  Ext.Ajax.request(
								  {
										url:    rootURL+'index.php/wf/UpdateTiming',
										method:  'POST',
										params: { 
					...
					,
					{
						...
						handler: function() {
									 ...
									 Ext.Ajax.request({
										...
										params: { 
											eventtype:'Видео',
											theprocess:commenturokid,
											processstatus:null,
											statetask:null,
											doer:null,
											thedoc:null,
											thevideo:filerecord.get('id'),
											thediscussion:null
										},
										success: function(response){
											var text = response.responseText;
						 ...
						 ,
						 listeners:{
							render: function(e) {Ext.QuickTips.register({  target: e.getEl(), text: 'Подписка на видео'});}
						},
						text:''
					}
					,
