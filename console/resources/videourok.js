		...
		store_iu_urok_docs.each(function(record,idx){
			if(record.get('doctype')=='{034C89DD-C5D9-4E48-9932-F9A6BD14A326}'){
				...
				if (record.get('fileurl')!=''){
					scn.setValue('<a href="' +record.get('fileurl') +'" target="_blank">Файл</a>');
				}else{
					scn.setValue('<a href="/output_file.php?ID=' +record.get('fileref') +'&ext=' +record.get('fileref_ext') +'" target="_blank">Файл</a>');
				}
			}
		});
		VUPanel.doLayout();
