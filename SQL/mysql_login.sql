...
set athe_session=null  ; 
 ...
 if aPWD is null then 
   ...
   select count(*) into existsCnt from iu_u_def join instance on iu_u_def.instanceid=instance.instanceid  where iu_u_def.Login=ausr  and iu_u_def.archived=0 and instance.archived=0; 
   if existsCNt =1   then 
		  select count(*) into existsCnt from users where Login=ausr and Password =MD5(aPWD); 
		  if existsCNt =1   then 
